...
if [ ! -f "$CORAL_HOME/config/config.toml" ];
then

  corald init ${MONIKER:-nonamenode} --home=${CORAL_HOME:-/.corald} --chain-id=${CHAIN_ID:-cosmwasm-coral}

  cd $CORAL_HOME/config
  ...
  if [ ! -z "$GENESIS_URL" ]; then
      ...
      wget https://raw.githubusercontent.com/CosmWasm/testnets/master/coralnet/config/genesis.json
  fi
...
# rm /etc/supervisor/conf.d/supervisor-coral.conf
# rm /etc/supervisor/conf.d/supervisor-corald.conf

cd /etc/supervisor/conf.d/

cat > supervisor-coral.conf << EOF
[program:coral]
command=coral rest-server --laddr tcp://0.0.0.0:${LCD_PORT:-1317} --home=${CORAL_HOME:-/.corald} --chain-id=${CHAIN_ID:-cosmwasm-coral} --trust-node --node=${RPC_LADDR:-tcp://0.0.0.0}:${RPC_PORT:-26657}
redirect_stderr=false
autostart=true
autorestart=unexpected
startsecs=10
startretries=5
stdout_logfile=/stdout-coral.txt
stdout_logfile_maxbytes=10MB
stdout_logfile_backups=10
stdout_capture_maxbytes=10MB
stdout_events_enabled=false
stderr_logfile=/stderr-coral.txt
...
cat > supervisor-corald.conf << EOF
[program:corald]
command=corald start --home=${CORAL_HOME:-/.corald}
redirect_stderr=false
autostart=true
autorestart=unexpected
startsecs=10
startretries=5
stdout_logfile=/stdout-corald.txt
stdout_logfile_maxbytes=10MB
stdout_logfile_backups=10
stdout_capture_maxbytes=10MB
stdout_events_enabled=false
stderr_logfile=/stderr-corald.txt
stderr_logfile_maxbytes=10MB
stderr_logfile_backups=10
stderr_capture_maxbytes=10MB
stderr_events_enabled=false
