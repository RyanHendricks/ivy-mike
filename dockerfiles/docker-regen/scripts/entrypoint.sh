...
if [ ! -f "$REGEN_HOME/config/config.toml" ];
then
    xrnd init ${MONIKER:-regen-node} --home=${REGEN_HOME:-/.xrnd} --chain-id=${CHAIN_ID:-regen-test-1001}
    ...
    if [ ! -z "$GENESIS_URL" ]; then
        ...
        wget https://raw.githubusercontent.com/regen-network/testnets/master/regen-test-1001/genesis.json
    fi
...
grpc_max_open_connections = ${GRPC_MAX_OPEN_CONNECTIONS:-1200}
...
seeds = "${SEEDS:-bcc81bbb45945307c8b32a276fa9700f1164f3b7@149.28.225.246:26656, caed8cafd56b23739aea8fdf339caaac5a453b8c@66.42.79.197:26656}"

# Comma separated list of nodes to keep persistent connections to
persistent_peers = "${PERSISTENT_PEERS:-bcc81bbb45945307c8b32a276fa9700f1164f3b7@149.28.225.246:26656, caed8cafd56b23739aea8fdf339caaac5a453b8c@66.42.79.197:26656}"
...
max_packet_msg_payload_size = ${MAX_PACKET_MSG_PAYLOAD_SIZE:-2000}
...
max_txs_bytes = ${MAX_TXS_BYTES:-1093741824}
...
if [ ! -z "$LCD_PORT" ]; then
    rm /etc/supervisor/conf.d/supervisor-xrncli.conf
    cd /etc/supervisor/conf.d/

cat > supervisor-xrncli.conf << EOF
[program:xrncli]
command=xrncli rest-server --laddr tcp://0.0.0.0:${LCD_PORT:-1317} --home=${REGEN_HOME:-/.regen} --chain-id=${CHAIN_ID:-regen-test-1001} --trust-node --node=tcp://localhost:${RPC_LADDR_PORT:-26657} --cors="${CORS_ALLOWED_ORIGINS:-*}"
